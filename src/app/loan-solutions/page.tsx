...
import { getLoanSolution } from './loan-solution-service-api';
...
const page = async () => {
  const loanSolutions = await getLoanSolution();
  return (
    <main className='flex min-h-screen flex-col items-center justify-center '>
      <HeroSection title='Loan Solutions' image={heroSectionTractorBg} />
      <ProductLine />
      {loanSolutions?.map((loanSolution, index) => {
        return (
          <Financing
            key={index}
            financeService={loanSolution}
